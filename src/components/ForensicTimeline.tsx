...
export default function ForensicTimeline({ events }: { events: TimelineEvent[] }) {
  ...
  return (
    <div className="relative pl-8 space-y-10">
      {/* The Line */}
      <div className="absolute left-3 top-2 bottom-2 w-px bg-gradient-to-b from-indigo-500/50 via-slate-800 to-transparent" />
      ...
      {events.map((event, idx) => (
        ...
        >
          {/* The Dot */}
          <div className={cn(
            "absolute -left-[25px] w-4 h-4 rounded-full border-4 border-black z-10 transition-transform group-hover:scale-125 shadow-[0_0_10px_rgba(0,0,0,0.5)]",
            event.severity === 'high' ? "bg-rose-500" : 
            event.severity === 'medium' ? "bg-amber-500" : "bg-emerald-500"
          )} />

          <div className="glass-card bg-white/[0.02] border-white/5 p-6 hover:bg-white/[0.04] transition-all hover:border-white/10">
            <div className="flex justify-between items-start mb-4">
               <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-white uppercase tracking-widest">{event.event_type.replace('_', ' ')}</h4>
